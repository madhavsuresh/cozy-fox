...
public struct MobilityProfile: Codable, Sendable, Hashable {
    ...
    /// One lightweight, semantic record of a commute leg. Stores route
    /// identity, anchors, and durations rather than a raw trace — the
    /// `PersonalAccessEstimator` reads this to learn "MY walk from MY
    /// home to MY Belmont station takes ~6 minutes" without ever
    /// retaining a GPS log.
    public struct CommuteLegObservation: Codable, Sendable, Hashable, Identifiable {
        ...
        public enum SampleQuality: String, Codable, Sendable, Hashable {
            ...
            /// Anchor-exit / anchor-entry inference without a precise
            /// boarding point.
            case inferred
        }
    ...
    public init(from decoder: Decoder) throws {
        ...
        if summary.isEmpty,
           ...
           !self.observations.isEmpty || !self.routeObservations.isEmpty || !self.commuteLegObservations.isEmpty
        {
...
public struct MobilityProfileSummary: Codable, Sendable, Hashable {
    ...
    /// Long-lived per-(direction × mode × route × stop × anchor pair)
    /// access-time aggregate, fed by `MobilityProfile.commuteLegObservations`
    /// and consumed by `PersonalAccessEstimator`. Welford-style running mean
    /// + M2 over access seconds (and optionally total trip seconds).
    ...
    /// Custom decoder so adding `commuteLegPatterns` /
    /// `consumedCommuteLegObservationCount` doesn't reject pre-existing
    /// summary blobs that were serialized before these fields existed.
    public init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
    ...
    /// Folds a single commute-leg observation into its
    /// `CommuteLegPattern` aggregate, creating one if it doesn't exist.
